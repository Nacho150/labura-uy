...
export async function getAdminProfiles() {
  ...
  const adminResponse = await fetch(restUrl("admin_users", "?select=user_id&limit=1"), {
    method: "GET",
    headers: getAuthHeaders(),
  });
  ...
  const response = await fetch(
    restUrl(
      "profiles",
      "?select=id,full_name,city,department,interested_categories,availability,recommended_jobs,created_at&order=created_at.desc",
    ),
    {
      method: "GET",
      headers: getAuthHeaders(),
